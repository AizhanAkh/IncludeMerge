...
enum definition_dmeff_interacting_species {
  ionized, /**<sum over free electrons and protons */
...
struct thermo
{
  ...
  double Tdmeff_initial; /* Tdmeff at z = initial_z_recfast */
  double Vrms_initial; /* Vrms at z = initial_z_recfast */
  short  dmeff_backreaction_on_baryons; /* do we incorporate backreaction onto baryon temperature?*/
  short  dmeff_baryon_heat_exchange; /* do we incorporate dmeff_baryon_heat_exchange?*/
  short  Tracy_and_Cora_Helium; /* do we use Tracy and Cora's prescription for Helium?*/
  ...
  int index_th_orig_dkappa_dmeff;  /**< dmeff momentum exchange rate, unmodified by relative velocity */
  int index_th_orig_dkappaT_dmeff; /**< dmeff heat exchange rate, unmodified by relative velocity */
...
struct recombination {
  ...
  int index_re_orig_dkappa_dmeff;  /**< dmeff momentum exchange rate, unmodified by relative velocity */
  int index_re_orig_dkappaT_dmeff; /**< dmeff heat exchange rate, unmodified by relative velocity */
  int index_re_heat_chi_dmeff;/**< dmeff heating from relative velocity dissipation, contribution to dmeff */
...
/**
 * Temporary structure to handle dmeff.
 *
 * This structure is used internally by the thermodynamics module,
 * but never passed to other modules.
 */
...
struct dmeff_thermo {
  ...
  /* Indices for temperature evolution. */
  int index_ti_tau;
  int index_ti_Tdm;
  int ti_size; /* number of integration indices */

  double zdec; /**< z at dmeff-baryon decoupling  */
  double Tdmeff_initial; /**< Tdmeff_initial at recfast_z_initial  */
  double Vrms_initial; /**< Vrms_initial at recfast_z_initial  */
...
/**
 * temporary  parameters and workspace passed to the dmeff_temperature_derivs function
 */

struct dmeff_parameters_and_workspace {
