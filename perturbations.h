...
struct perturbs
{
  ...
  int dmeff_niter_dynamic_max; /**< number of times perturbations are solved for */

  /* Vrms, Vflow, and V^2 integrand for dmeff.
   Use background table for tau for tau values to save. [index_tau * ppt->k_size + index_k] */
...
extern "C" {
  ...
  int perturb_dmeff_Vrms_dynamic_fill(
                                      struct precision * ppr,
                                      struct background * pba,
                                      struct thermo * pth,
                                      struct perturbs * ppt,
