...
struct background
{
  ...
  double ratio_dmeff2cdm; /**< \f$ \f_{dmeff} \f$ : fraction of dark matter with effective interactions */
  int index_dmeff_niter; /**< index counting the number of iterations in perturbations; used in thermodynamics but thermo structure is freed when interating, so strored in background*/

  double sigma_dmeff; /**< \f$ \simga_{dmeff} \f$ : cross section of dark matter with effective interactions, in units of cm^2 */

  double Vrel_dmeff_rate; /**< \f$ \Vrel_{dmeff} \f$ : Relative velocity between DM an baryons entering in the momentum and heat exchange rates */
  double npow_dmeff; /**< \f$ \n_{dmeff} \f$ : power of velocity of cross section for dark matter with effective interactions with protons */
  ...
  int index_bg_Vrms;          /**< dmeff Vrms (not estimated, updated in thermodynamics) */
  int index_bg_dkappa_dmeff;  /**< dmeff dkappa/dtau (estimate only, if T_b=T_photon always) */
  int index_bg_dkappaT_dmeff; /**< dmeff dkappa/dtau, for temperature (estimate only, if T_b=T_photon always) */
  int index_bg_orig_dkappa_dmeff;  /**< dmeff momentum exchange rate, unmodified by relative velocity (estimate only, if T_b=T_photon always) */
  int index_bg_orig_dkappaT_dmeff; /**< dmeff heat exchange rate, unmodified by relative velocity (estimate only, if T_b=T_photon always) */
